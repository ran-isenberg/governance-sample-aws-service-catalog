...
mypy-lint:
	poetry run mypy --pretty catalog_backend cdk tests
...
unit:
	poetry run pytest tests/unit  --cov-config=.coveragerc --cov=catalog_backend --cov-report xml
...
integration:
	poetry run pytest tests/integration  --cov-config=.coveragerc --cov=catalog_backend --cov-report xml
